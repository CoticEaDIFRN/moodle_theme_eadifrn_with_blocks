 ...
 * @return string
 */
function theme_boost_eadifrn_with_blocks_get_main_scss_content($theme) {
    global $CFG;
    ...
    // $pre = file_get_contents($CFG->dirroot . '/theme/boost_eadifrn_with_blocks/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    // $post = file_get_contents($CFG->dirroot . '/theme/boost_eadifrn_with_blocks/scss/post.scss');
 ...
 * @return array
 */
function theme_boost_eadifrn_with_blocks_get_pre_scss($theme) {
    global $CFG;
    // MODIFICATION START.
    require_once($CFG->dirroot . '/theme/boost_eadifrn_with_blocks/locallib.php');
    // MODIFICATION END.
    ...
    // $scss .= theme_boost_eadifrn_with_blocks_get_loginbackgroundimage_scss();
    // MODIFICATION END.
 ...
 * @return bool
 */
function theme_boost_eadifrn_with_blocks_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $theme = theme_config::load('boost_eadifrn_with_blocks');
        if ($filearea === 'favicon') {
...
/**
 * If setting is updated, use this callback to clear the theme_boost_eadifrn_with_blocks' own application cache.
 */
...
function get_ead_ifrn_with_blocks_commom_moodle_template_context()
{
...
function get_ead_ifrn_with_blocks_calendario() {
    global $CFG, $COURSE;
...
function get_ead_ifrn_with_blocks_course_content_actions()
{
    global $PAGE, $COURSE;
...
function get_ead_ifrn_with_blocks_course_common_actions() 
{
    global $PAGE, $COURSE;
    if ($PAGE->pagelayout == "course" || $PAGE->pagelayout == "incourse") {
        $extraflatnav = [];
...
function get_ead_ifrn_with_blocks_template_context()
{
    global $PAGE;

    $templatecontext = get_ead_ifrn_with_blocks_commom_moodle_template_context();

    if ($templatecontext['in_course_page'] || $templatecontext['within_course_page']) {
        $templatecontext['course_content_actions'] = get_ead_ifrn_with_blocks_course_content_actions();
        $templatecontext['course_common_actions'] = get_ead_ifrn_with_blocks_course_common_actions();
    }
    $templatecontext['nosso_calendario'] = get_ead_ifrn_with_blocks_calendario();
