...
//  * @return string
//  */
// function theme_boost_eadifrn_with_blocks_get_random_loginbackgroundimage_class() {
//
//     // Fetch context.
...
//     $files = $fs->get_area_files($systemcontext->id, 'theme_boost_eadifrn_with_blocks', 'loginbackgroundimage', false, 'itemid', false);
//
...
//  * @return string
//  */
// function theme_boost_eadifrn_with_blocks_get_loginbackgroundimage_scss() {
//     $count = 0;
//     $scss = "";
//
//     // Fetch context.
...
//         $fs = get_file_storage();
//         // Get all files from filearea.
//         $files = $fs->get_area_files($systemcontext->id, 'theme_boost_eadifrn_with_blocks', 'imageareaitems', false, 'itemid', false);
//
...
//             $lines = explode("\n", get_config('theme_boost_eadifrn_with_blocks', 'imageareaitemslink'));
//             // Parse item settings.
 ...
 */
function theme_boost_eadifrn_with_blocks_process_flatnav(flat_navigation $flatnav) {
    global $USER;
    // If the setting defaulthomepageontop is enabled.
    if (get_config('theme_boost_eadifrn_with_blocks', 'defaulthomepageontop') == 'yes') {
        ...
        if (($coursehomenode = $flatnav->find('coursehome', global_navigation::TYPE_CUSTOM)) != false) {
            ...
            if (get_config('core', 'defaulthomepage') == HOMEPAGE_SITE) {
                ...
                $flatnavreturn = theme_boost_eadifrn_with_blocks_set_node_on_top($flatnav, 'home', $coursehomenode);
            } else if (get_config('core', 'defaulthomepage') == HOMEPAGE_MY) { // If the dashboard is set as the default homepage
                // by the admin.
                // Return the modified flat_navigtation.
                $flatnavreturn = theme_boost_eadifrn_with_blocks_set_node_on_top($flatnav, 'myhome', $coursehomenode);
 ...
 * @return flat_navigation.
 */
function theme_boost_eadifrn_with_blocks_set_node_on_top(flat_navigation $flatnav, $nodename, $beforenode) {
 ...
 */
function theme_boost_eadifrn_with_blocks_get_incourse_settings() {
    global $COURSE, $PAGE;
    // Initialize the node with false to prevent problems on pages that do not have a courseadmin node.
    $node = false;
    // If setting showsettingsincourse is enabled.
    if (get_config('theme_boost_eadifrn_with_blocks', 'showsettingsincourse') == 'yes') {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE || $PAGE->context->contextlevel == CONTEXT_MODULE) {
            ...
            if (!empty($node)) {
                ...
                if (get_config('theme_boost_eadifrn_with_blocks', 'incoursesettingsswitchtorole') == 'yes' && !is_role_switched($COURSE->id)) {
                    ...
                    if (is_array($roles) && (count($roles) > 0)) {
                        ...
                        $properties = array('text' => get_string('switchroleto', 'theme_boost_eadifrn_with_blocks'),
 ...
 * @return navigation_node.
 */
function theme_boost_eadifrn_with_blocks_get_incourse_activity_settings() {
    global $PAGE;
